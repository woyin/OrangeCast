...
func (srv *Server) protectedRoutes() *http.ServeMux {
	...
	mux.HandleFunc("/workbench/discovery-settings", srv.handleDiscoverySettings)
	mux.HandleFunc("/workbench/creation-proposals/accept", srv.handleCreationProposalAccept)
	mux.HandleFunc("/workbench/creation-history", srv.handleCreationHistoryCreate)
	mux.HandleFunc("/workbench/ideation", srv.handleIdeationSessionCreate)
	...
	mux.HandleFunc("/workbench/curate", srv.handleCuratorRun)
	...
	mux.HandleFunc("/documents/import", srv.handleDocumentImport)
	mux.HandleFunc("/documents/pdf", srv.handleDocumentPDF)
	...
	mux.HandleFunc("/api/keypoints/quality", srv.handleKeyPointQuality)
	mux.HandleFunc("/api/keypoints/batch-status", srv.handleKeyPointBatchStatus)
	...
	mux.HandleFunc("/api/material-candidates/promote", srv.handleMaterialCandidatePromote)
	mux.HandleFunc("/api/rights-constraints", srv.handleRightsConstraint)
