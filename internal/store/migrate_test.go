...
func TestMigrate_V01FixtureUpgrade(t *testing.T) {
	...
	if len(applied) != 8 {
		t.Fatalf("应应用 8 条迁移，实际 %d", len(applied))
	}
	after := countAll(t, db)
...
func TestMigrate_FailedMigration_SafeRetry(t *testing.T) {
	...
	bad := migration{version: 10, name: "0007_broken", up: "CREATE TABLE boom (id INTEGER); SELECT no_such_column FROM no_such_table;"}
	...
	if v != 8 {
		t.Errorf("失败迁移不应登记版本；应保持 9，实际 %d", v)
	}
