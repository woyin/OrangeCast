...
func (s *Store) IndexKeyPoints(ctx context.Context, sourceType models.SourceType, sourceID, sourceTitle string, cardVersion int, card *provider.KnowledgeCard, segments []provider.Segment) error {
	...
	for _, kp := range card.KeyPoints {
		...
		if _, err := tx.ExecContext(ctx,
			...
			string(citationsJSON), string(models.RelationCitation), start, end, cardVersion); err != nil {
...
func (s *Store) ListKeyPoints(ctx context.Context, page, perPage int) ([]*KeyPointRow, int, error) {
	...
	rows, err := s.DB.QueryContext(ctx,
		`SELECT id, source_type, source_id, source_title, content, description, citations_json, relation_kind, time_start, time_end, card_version, created_at
...
func (s *Store) SearchKeyPoints(ctx context.Context, query string, page, perPage int) ([]*KeyPointRow, int, error) {
	...
	offset := (page - 1) * perPage
	rows, err := s.DB.QueryContext(ctx,
		`SELECT ki.id, ki.source_type, ki.source_id, ki.source_title, ki.content, ki.description, ki.citations_json, ki.relation_kind, ki.time_start, ki.time_end, ki.card_version, ki.created_at
...
func scanKeyPointRows(rows *sql.Rows) ([]*KeyPointRow, int, error) {
	...
	for rows.Next() {
		r := &KeyPointRow{}
		var rk string
		if err := rows.Scan(&r.ID, &r.SourceType, &r.SourceID, &r.SourceTitle, &r.Content, &r.Description, &r.CitationsJSON, &rk, &r.TimeStart, &r.TimeEnd, &r.CardVersion, &r.CreatedAt); err != nil {
			return nil, 0, err
		}
