...
测试覆盖（341+ 测试函数）：迁移（含数据修复 0015、破坏性迁移单 Owner 守卫与备份）、备份/恢复 E2E（含 Create/Restore 错误路径与 DB 哈希校验）、Owner 认领、CSRF/限流、safehttp SSRF 防护（safeDialer 私网拦截）、EvidenceAudio、worker 崩溃恢复（含 Run 主循环/doHighlight/fetchRawAudio/downloadAudio/默认 bundleFor/guessAudioExt）、Citation 校验、EvalSet（含 CheckReferenceSamples/quoteInSegments/allCitations）、Markdown 渲染（含 fmtTime/frontmatterValue/referenceLinks）、分段搜索、EvidenceQA/StudyChat 完整 handler、StudyChat 硬约束、Paraphrase 最近 N 次淘汰、Narration 合成（含 runCLI/nextNarrationVersion/NewKokoroProvider 默认值）、Highlight 稳定 ID、DJ 页渲染、Narration serve、批量入队、CLI 分发（parseCommand）、RSS 抓取与解析（FetchFeed/parseFeed/Refresher）、auth 中间件（含会话 cookie/限流/CSRF/RequireAuth）、store 生命周期与迁移安全（含 Open 多用户守卫/ConsistencyBackup/sqlQuoteString）、server 全部 handler（含 source 详情/版本回退/音频回退/DJ 404/Collection 校验）、config 目录创建、provider HTTP 层与模型路径（经 WithBaseURL + httptest 隔离）。各包覆盖率：markdown 98%、config 97%、auth 92%、safehttp 92%、provider 87%、evalset 86%、rss 83%、server 82%、store 81%、queue 80%、backup 79%。
