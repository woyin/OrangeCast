...
测试覆盖（380+ 测试函数）：迁移（含数据修复 0015、破坏性迁移单 Owner 守卫与备份）、备份/恢复 E2E（含 Create/Restore 错误路径、DB/证据哈希校验、缺失证据、非 gzip 包）、Owner 认领、注册（弱密码/非法邮箱拒绝）、登录（成功/错误密码/限流）、CSRF/限流、safehttp SSRF 防护（safeDialer 私网拦截）、EvidenceAudio、worker 崩溃恢复（含 Run 主循环/doHighlight/fetchRawAudio/downloadAudio/默认 bundleFor/guessAudioExt/fileSHA256/HighlightName）、Citation 校验、EvalSet（含 CheckReferenceSamples/quoteInSegments/allCitations）、Markdown 渲染（含 fmtTime/frontmatterValue/referenceLinks）、分段搜索、EvidenceQA（含拒答/有引用/405）、StudyChat 完整 handler（含长标题截断/会话复用/生成失败）、StudyChat 硬约束、Paraphrase（含参考片段不存在/无片段 400）、Narration 合成（含 runCLI/nextNarrationVersion/NewKokoroProvider 默认值）、Highlight 稳定 ID、DJ 页渲染（含无高光/无转录稿 404）、Narration serve（含记录/无记录缺文件 404）、批量入队（含 405）、上传（正常/拒绝类型/缺文件/multipart 解析失败）、Markdown 下载（含 with_generated/卡片损坏 500）、模板渲染（含未知模板错误/formatSeconds）、CLI 分发与参数解析（parseCommand/ensureArchiveExt）、RSS 抓取与解析（FetchFeed/parseFeed/Refresher/runScheduled/发布时间）、auth 中间件（含会话 cookie/限流/CSRF/RequireAuth）、store 生命周期与迁移安全（含 Open 多用户守卫/非法路径/ConsistencyBackup/sqlQuoteString/SourceTitle/Status/IndexSearch 错误/ListKeyPoints 分页/GetJob/GetPodcastByID/GetUserByEmail）、server 全部 handler（含 source 详情/版本页 episode+upload/回退/音频回退/播客详情/播客列表 500/图谱 API/KeyPoint 分页/Collection 校验/parseSegmentIDs）、config 目录创建、provider HTTP 层与模型路径（经 WithBaseURL + httptest 隔离）。各包覆盖率：markdown 98%、config 97%、auth 92%、safehttp 92%、provider 87%、rss 88%、evalset 86%、server 85%、store 82%、queue 80%、backup 79%。
